...
- [ ] 1.6 `src/lib/agentUtils.ts` を作成（カンマ区切りエージェント名のパース関数）
- [ ] 1.7 `src/app/api/admin/agents/route.ts` を更新（`lang` クエリパラメータ対応、多言語名返却）
- [ ] 1.8 `src/app/api/agent/route.ts` の `loadRegistry` を更新（言語対応）
- [ ] 1.9 `src/components/AgentPicker.tsx` を更新（`lang` パラメータ付きAPI呼び出し、言語変更時の再取得）
...
- [ ] 2.5 エージェント名が選択言語で表示される
- [ ] 2.6 言語切替時にエージェント名も更新される
- [ ] 2.7 単一言語のエージェント名（カンマなし）も正常に動作する
